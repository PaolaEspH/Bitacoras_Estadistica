...
\begin{figure}[h]
\centering
\includegraphics[width=1\linewidth]{resumen_base.png}
\caption{\label{fig:resumen de la base de datos}resumen de la base de datos}
\end{figure}

De manera que, al hacer el proceso anterior, tenemos que la base de datos contiene un total de 33,708 observaciones y 10 variables. Las variables incluyen tanto características numéricas como categóricas relacionadas con accidentes de tráfico. Entre las variables numéricas se encuentran (velocidad\_recomendada), "fatales", "lesiones\_menores", "lesiones\_graves" y "limite\_velocidad", que representan las condiciones de los accidentes, tales como la velocidad recomendada, el número de víctimas fatales y heridas, y los límites de velocidad en las zonas de los accidentes. Por otro lado, las variables categóricas incluyen "severidad" (que clasifica los accidentes según su gravedad), "festivos" (que indica si el accidente ocurrió en un día festivo o no), "iluminación" (que describe las condiciones de luz al momento del accidente), "carretera" (que indica el tipo de carretera involucrada en el accidente), y "clima" (que especifica las condiciones climáticas en el momento del accidente). Esta información permite realizar un análisis detallado de las condiciones y características de los accidentes en la base de datos, lo que facilita identificar patrones y factores que influyen en la severidad de los accidentes.
...
\subsubsection{Ficha 1}

\textbf{Nombre de su hallazgo:} Limitaciones de la correlación para variables categóricas

\textbf{Resumen en una oración:} La correlación no es adecuada para analizar independencia entre variables categóricas sin un orden natural claro.

\textbf{Principal característica:} El análisis de independencia entre variables categóricas debe realizarse con tablas de contingencia y pruebas como el chi-cuadrado.

\textbf{Problemas o posibles desafíos:} La interpretación errónea de coeficientes de correlación en variables categóricas puede llevar a conclusiones engañosas si no se considera la naturaleza ordinal o nominal de las categorías.

\textbf{Resumen en un párrafo:} Aunque puede calcularse la correlación entre variables categóricas codificadas numéricamente, esta medida no siempre refleja relaciones reales, especialmente si las categorías no tienen un orden natural y bien definido. Por ello, el análisis exploratorio mediante correlación debe tomarse con precaución. Para evaluar la independencia entre variables categóricas, es más apropiado utilizar herramientas diseñadas para este propósito, como las tablas de contingencia combinadas con pruebas estadísticas como el chi-cuadrado, que consideran explícitamente las frecuencias observadas y esperadas entre combinaciones de categorías.

\subsubsection{Ficha 2}

\textbf{Nombre de su hallazgo:} Relaciones lineales entre variables numéricas en accidentes de tránsito

\textbf{Resumen en una oración:} La matriz de correlación revela una fuerte asociación entre velocidad recomendada y límite de velocidad, y una débil relación entre víctimas fatales y con lesiones graves.

\textbf{Principal característica:} Se identifican patrones lineales entre variables cuantitativas mediante el coeficiente de Pearson, destacando asociaciones claras en medidas de velocidad y ambigüedad en indicadores de severidad.

\textbf{Problemas o posibles desafíos:} Las bajas correlaciones entre indicadores de gravedad pueden dificultar el modelado conjunto de estos factores y reflejar la influencia de variables externas no observadas.

\textbf{Resumen en un párrafo:} El análisis de la matriz de correlación para las variables numéricas revela información clave sobre las relaciones entre distintos aspectos de los accidentes de tránsito. Una fuerte correlación positiva entre la velocidad recomendada y el límite de velocidad sugiere una coherencia entre las condiciones del camino y la regulación vial. Por el contrario, la débil correlación entre el número de víctimas fatales y las lesiones graves pone de manifiesto que la severidad de un accidente no siempre se manifiesta de forma proporcional en distintos tipos de daño, lo que podría estar influido por factores como el tipo de accidente, la infraestructura o el tiempo de respuesta ante emergencias. Además, se anticipa la exploración gráfica de variables categóricas como la severidad del accidente, utilizando gráficos de barras para facilitar la comprensión visual de su distribución.

\subsubsection{Ficha 3}

\textbf{Nombre de su hallazgo:} Distribución de la severidad de accidentes

\textbf{Resumen en una oración:} La mayoría de los accidentes en la base de datos son de severidad leve o sin heridos, siendo menos frecuentes los casos graves o fatales.

\textbf{Principal característica:} El análisis gráfico muestra una alta frecuencia de accidentes menores frente a una baja pero significativa presencia de eventos severos.

\textbf{Problemas o posibles desafíos:} La baja frecuencia de accidentes graves puede dificultar su análisis estadístico detallado y puede requerir enfoques específicos para estudiar eventos raros.

\textbf{Resumen en un párrafo:} El gráfico de barras sobre la severidad de los accidentes permite observar que los incidentes más comunes en la base de datos son aquellos sin heridos o con consecuencias leves, mientras que los accidentes con víctimas graves o fatales son significativamente menos frecuentes. Esta asimetría en la distribución indica que, aunque los accidentes viales ocurren con regularidad, en la mayoría de los casos no resultan en consecuencias extremas. No obstante, la presencia de accidentes fatales, aunque baja, representa una preocupación crítica en términos de política pública y prevención, y justifica un análisis más profundo de los factores que los provocan.

\subsubsection{Ficha 4}

\textbf{Nombre del hallazgo:} Asociación entre clima y severidad de accidentes en términos relativos y absolutos

\textbf{Resumen en una oración:} Las condiciones climáticas influyen tanto en la proporción como en la cantidad absoluta de accidentes, siendo el buen clima asociado con más accidentes severos en proporción y en número total.

\textbf{Principal característica:} El análisis conjunto de gráficos de barras apiladas (proporcional y absoluto) permite identificar que, aunque en todas las condiciones climáticas predominan los accidentes sin lesiones, el buen clima concentra no solo más accidentes en total, sino también una mayor proporción de los más graves.

\textbf{Problemas o posibles desafíos:} El análisis proporcional puede inducir a interpretaciones erróneas si no se complementa con datos absolutos, ya que no considera la magnitud total de los eventos por categoría climática.

\textbf{Resumen en un párrafo:} El análisis de la severidad de los accidentes según el clima revela, en términos proporcionales, que los accidentes sin lesiones son los más frecuentes en todos los tipos de clima, mientras que el buen clima presenta una proporción relativamente mayor de accidentes graves y fatales. Esto puede deberse a una mayor confianza al conducir o mayor flujo vehicular. Al considerar los datos absolutos, se confirma que el buen clima también registra el mayor número total de accidentes, incluidos los severos, lo cual refuerza la hipótesis de una mayor exposición al riesgo en condiciones favorables. En contraste, condiciones adversas como nieve o niebla muestran tanto una menor proporción como una menor cantidad absoluta de accidentes severos. Este enfoque dual permite una comprensión más profunda del papel del clima en la ocurrencia y gravedad de los accidentes, alertando sobre la importancia de no subestimar el riesgo en días despejados.

\subsubsection{Ficha 5}

\textbf{Nombre del hallazgo:} Mayor proporción y concentración de accidentes severos durante festividades específicas

\textbf{Resumen en una oración:} Los días festivos presentan una mayor proporción y, en ciertos casos, una alta concentración absoluta de accidentes graves y fatales, en comparación con los días no festivos.

\textbf{Principal característica:} Al analizar tanto proporciones como volúmenes absolutos de accidentes por festividad, se identifican fechas específicas (como Navidad, Año Nuevo o el Fin de Semana del Trabajo) en las que aumenta significativamente el riesgo de siniestros severos.

\textbf{Problemas o posibles desafíos:} La categoría “No festivo”, al representar la mayoría de días del año, puede ocultar tendencias importantes en las fechas especiales si no se analiza por separado o se ajusta la escala del gráfico.

\textbf{Resumen en un párrafo:} El análisis facetado de la severidad de los accidentes por tipo de festividad revela que, si bien los accidentes sin lesiones predominan, las festividades tienden a mostrar una proporción mayor de casos graves y fatales respecto a los días no festivos. En particular, fechas como Navidad, Año Nuevo y el Fin de Semana del Trabajo concentran un volumen notable de accidentes severos, lo que sugiere un aumento del riesgo vial posiblemente relacionado con celebraciones, desplazamientos masivos o consumo de alcohol. La visualización de los datos absolutos y el posterior filtrado de los días no festivos permiten una comparación más clara entre festividades, destacando que ciertas fechas, aunque poco frecuentes, implican un riesgo desproporcionadamente alto. Este hallazgo resalta la necesidad de medidas de prevención focalizadas en fechas festivas de alto riesgo.
